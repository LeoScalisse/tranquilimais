...
const WellnessTipCard: React.FC<{
  ...
  refreshCount: number;
  maxRefreshes: number;
}> = ({ tipData, isLoading, onRefresh, refreshCount, maxRefreshes }) => {
  return (
                ...
                <button
                  ...
                  className={`p-1.5 rounded-full transition-all duration-300 ${
                    refreshCount >= maxRefreshes
                      ...
                      : 'text-tranquili-blue hover:bg-blue-50 hover:rotate-180'
                  }`}
                  title="Nova dica"
                >
...
const GratitudeJournal: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (stored) {
      try {
        ...
        const todayEntry = allEvents.find((e: any) =>
          e.category === "Gratidão" &&
  ...
  if (savedToday) {
    return (
      <div className="bg-gradient-to-br from-yellow-50 to-orange-50 p-4 rounded-xl shadow-sm border border-yellow-100 mb-6">
  ...
  return (
    <div className="bg-gradient-to-br from-yellow-50 to-orange-50 p-4 rounded-xl shadow-sm border border-yellow-100 mb-6">
      ...
      <textarea
        ...
        placeholder="Escreva aqui algo que te fez feliz..."
        className="w-full p-3 rounded-lg border border-yellow-200 bg-white/80 text-gray-800 text-sm resize-none focus:ring-2 focus:ring-yellow-300 outline-none"
        rows={2}
      />
      <button
        onClick={handleSave}
        disabled={!entry.trim()}
        className={`mt-2 w-full py-2 rounded-lg font-medium text-sm transition-all ${
          entry.trim()
...
const HomeScreen: React.FC<HomeScreenProps> = ({
  ...
  useEffect(() => {
    if (userProfile?.name) {
      const GREETINGS = [
        `Olá, ${userProfile.name}. Como você está?`,
  ...
  return (
    <div className="p-4 pb-28 bg-gray-50 h-full overflow-y-auto">
      ...
      <WellnessTipCard
        ...
        onRefresh={() => dailyRefreshCount < MAX_REFRESHES_PER_DAY && (playSound('select'), fetchTip(true))}
